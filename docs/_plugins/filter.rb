module Jekyll
    module Filter
        ...
        # Sample: {% assign categories = site.categories | sort_array_by_name: "ASC" %}
        # Sample: {% assign tags = site.tags | sort_array_by_name: "ASC" %}
        def sort_array_by_name(array, order="ASC")
            sorted_array_by_name = array.sort_by { | item, posts | [item.downcase] }

            case order.upcase
                when "ASC"
                    sorted_array_by_name_with_order = sorted_array_by_name
                when "DESC"
                    sorted_array_by_name_with_order = sorted_array_by_name.reverse
                else
                    sorted_array_by_name_with_order = array
            end

            return sorted_array_by_name_with_order
        end

        # 配列(カテゴリやタグなど)を記事数でソートする
        # Sample: {% assign categories = site.categories | sort_array_by_num_of_posts: "ASC" %}
        # Sample: {% assign tags = site.tags | sort_array_by_num_of_posts: "ASC" %}
        def sort_array_by_num_of_posts(array, order="ASC")
            case order.upcase
                when "ASC"
                    sorted_array_by_num_of_posts = array.sort_by { | item, posts | [posts.size, item.downcase] }
                when "DESC"
                    sorted_array_by_num_of_posts = array.sort_by { | item, posts | [-posts.size, item.downcase] }
                else
                    sorted_array_by_num_of_posts = array
            end

            return sorted_array_by_num_of_posts
        end

        # URLからページナビ配列を生成する
        # Sample: {% assign page_navi_array = page.url | generate_page_navi: page.categories, site.data.words %}
        ...
        # 読了時間を取得する
        # Sample: {{ include.post-obj.content | strip_html | strip_newlines | replace: "    ", "" | xml_escape | get_reading_time }}
        ...
        # 正規表現で文字列を置換する
        # Sample: {% assign str = "Hello <b>maku</b>" | replace_by_regexp: "<(.+?)>", "[\1]" %}

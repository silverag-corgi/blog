...
$(function() { // HTML要素読み込み待機
    ...
    $.getJSON("{{ '/data/search.json' | relative_url }}").done(function(docs) {
        // インデックス作成
        var index = lunr(function(index) {
            ...
            index.field("pub_at",  { boost:  0 });
            index.field("upd_at",  { boost:  0 });
    ...
    function generateSearchResult(item) {
        ...
        if(item === null) {
            ...
            searchResult += "                        <span><time datetime=\""+item.pub_at+"\">"+item.pub_at+"</time></span>\r\n";
            searchResult += "                    </li>\r\n";
            if(item.upd_at) {
            searchResult += "                    <li>\r\n";
            searchResult += "                        <i class=\"fa fa-fw fa-edit\"></i>\r\n";
            searchResult += "                        <span><time datetime=\""+item.upd_at+"\">"+item.upd_at+"</time></span>\r\n";
            searchResult += "                    </li>\r\n";
            }
            searchResult += "                </ul>\r\n";

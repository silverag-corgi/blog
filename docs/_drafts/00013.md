...
| ---------------- | ---------------------------------- | ------- |
| マウントポイント | `/`                                | (1)     |
| ブートローダ     | Linux用SSDのディスク (`/dev/sdXN`) | (2),(3) |

- (1) そのパーティションをLinux上のどの場所として使うかを表す
- (2) どのOSが起動時に優先されるかを表す
- (3) パーティションではなくディスクを指定する
...
| Linux1    | Linux EFI                          |          |
| Linux2    | Linux EFI                          |          |
| Windows   | Windows EFI (Windows Boot Manager) |          |
| BootOrder | Linux1,Linux2,Windows              | 起動順序 |
